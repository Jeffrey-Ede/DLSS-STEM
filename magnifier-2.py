...
cropsize = 512
d = 5#np.sqrt(40)
lq_size = (cropsize - (cropsize%d)//2) // d + 1
...
def load_image(addr, resize_size=cropsize, img_type=np.float32):
    ...
    try:
        ...
        img = np.zeros((cropsize,cropsize,1))
...
def create_example(img0, d=5):

    lq = np.zeros((lq_size, lq_size, 1))
    for i, x in enumerate(range((cropsize%d)//2, cropsize, d)):
        for j, y in enumerate(range((cropsize%d)//2, cropsize, d)):
            lq[j,i] = img0[y,x]

    lq, (min, max) = norm_img(lq, get_min_and_max=True)
    img = norm_img(img0, min, max)

    img = cv2.GaussianBlur(img,(3,3), 1.5)
    img = img[..., np.newaxis]

    #Changed img to img0 halfway through training
    return img.astype(np.float32), lq.astype(np.float32)

def record_parser(record):
    """Parse files and generate lower quality images from them."""

    img = flip_rotate(preprocess(load_image(record)))
    img = img[:cropsize, :cropsize]
    img, lq = create_example(img)

    if np.sum(np.isfinite(img)) != cropsize**2 or np.sum(np.isfinite(lq)) != lq_size**2:
        img = np.zeros((cropsize,cropsize, 1))
        lq = np.zeros((lq_size,lq_size, 1))
...
def shaper(lq, img):
    ...
    lq.set_shape([lq_size, lq_size, 1])
    img.set_shape([cropsize, cropsize, 1])
...
def residual_block(inputs, skip=2, layer=None, is_training=True):
...
def generator(inputs, num_outputs, is_training, is_depthwise_sep=False):
    ...
    mag = (cropsize/16)/lq_size #5x magnification  
    x = inputs
    ...
    x = conv(
        x, 
        num_outputs=64, 
        stride=[int(np.around(mag**(0.5)*x)) for x in inputs.get_shape().as_list()[1:3]],
    ...
    x = conv(
        x, 
        num_outputs=128, 
        stride=[int(np.around(mag*x)) for x in inputs.get_shape().as_list()[1:3]],
        is_depthwise_sep=is_depthwise_sep,
        is_training=is_training,
        transpose=True
    )
    ...
    #Residual blocks
    def layer(inputs, num_outputs, is_training):
        return conv(
            inputs=inputs, 
            num_outputs=num_outputs,
            is_depthwise_sep=is_depthwise_sep,
...
def configure(
  ...
  output = generator(
      inputs=small_images, 
      num_outputs=large_images.get_shape().as_list()[-1], 
      is_training=is_training,
      is_depthwise_sep=is_depthwise_sep
  )
  
  #Mean squared errors
  mse = 200*tf.reduce_mean( (output - large_images)**2, axis=[1,2,3] )
...
def experiment(report_every_n=100):
  ...
  def load_data_subset(subset):
      return load_data(
          dir="//Desktop-sa1evjv/f/ARM_scans-crops/",
          subset=subset, 
          batch_size=16
          )

  small_images, large_images = tf.case(
  ...
  first_small_image = small_images[0]
  ...
  model_dir = "//flexo.ads.warwick.ac.uk/Shared41/Microscopy/Jeffrey-Ede/models/magnifier/"

  saver = tf.train.Saver(max_to_keep=2)

  log_filepath = model_dir + "log-2.txt"
  ...
  with tf.Session(config=config) as sess, open(log_filepath, "a") as log_file:
    ...
    for iter in range(start_iter, train_iters):
      ...
      lr = learning_rate * 0.5**(min(iter//12_500, 2))
      ...
      if 0 <= iter <= 1 or not iter % 25_000 or (0 <= iter < 10_000 and not iter % 1000) or iter == start_iter:
        _, step_loss, [step_small_image, step_large_image, step_output] = sess.run([
            train_op, 
            loss,
            [first_small_image, first_large_image, first_output]
            ],
            feed_dict=feed_dict)
        ...
        Image.fromarray((0.5*step_small_image+0.5).reshape(lq_size, lq_size).astype(np.float32)).save( save_input_loc )
        Image.fromarray((0.5*step_large_image+0.5).reshape(cropsize, cropsize).astype(np.float32)).save( save_truth_loc )
        Image.fromarray((0.5*step_output+0.5).reshape(cropsize, cropsize).astype(np.float32)).save( save_output_loc )
      else:
