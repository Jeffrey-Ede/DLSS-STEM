...
def experiment(feature, ground_truth, mask, learning_rate_ph, discr_lr_ph, beta1_ph, 
    ...
    #Predict MSE
    with tf.variable_scope("mse_predicter"):
        m = tf.concat([tf.random_crop(truth, size=(batch_size, 5, 5, 1)) for _ in range(20)], axis=0)
        m = tf.contrib.layers.fully_connected(inputs=m, num_outputs=100, activation_fn=tf.nn.relu)
        m = tf.contrib.layers.fully_connected(inputs=m, num_outputs=200, activation_fn=tf.nn.relu)
        m = tf.contrib.layers.fully_connected(inputs=m, num_outputs=1, activation_fn=None)
        m = tf.reduce_mean(m, axis=[1,2,3])

    mse_inner = 200*adjusted_mse(blur_truth_small, output_inner)
    mse_inner, _ = capper_fn(mse_inner/loss_scale_ph)
    #mse_inner = 2.*tf.cond( mse_inner < 1, lambda: mse_inner, lambda: tf.sqrt(mse_inner+1.e-8) )
    #mse_inner = tf.minimum(mse_inner, 50)

    output_outer = tf.where(mask == 1, truth, output_outer)

    mse_outer = 200*adjusted_mse(blur_truth, output_outer)
    mse0 = mse_outer
    mse_outer, mean_m = capper_fn(mse_outer/loss_scale_ph)
    ...
    mse_outer_together, _ = capper_fn(mse_outer_together/loss_scale_ph)
    ...
    m_loss = tf.reduce_mean( ((mse0/loss_scale_ph) - m)**2 )

    loss = mse_outer_together + 2*wass_loss_for_gen_outer
    loss_inner = mse_inner 
    loss_outer = mean_m * mse_outer / tf.maximum(m, 0.1)

    train_ops_discr = []
    for i in range(3):
        d_loss = tf.cond( train_outer_ph, lambda: d_losses_outer[i], lambda: d_losses_inner[i] )
        d_train_op = tf.train.AdamOptimizer(discr_lr_ph, 0.5).minimize(
    ...
    train_op_trainer = tf.train.AdamOptimizer(learning_rate_ph, beta1_ph).minimize(
        2*loss_inner, var_list=model_params_trainer)

    train_op_inner_start = tf.train.AdamOptimizer(learning_rate_ph, beta1_ph).minimize(
        loss_inner+loss_outer, var_list=model_params_inner)
    train_op_inner_end = tf.train.AdamOptimizer(learning_rate_ph, beta1_ph).minimize(
        loss_inner+loss, var_list=model_params_inner)

    train_op_outer_start = tf.train.AdamOptimizer(learning_rate_ph, beta1_ph).minimize(
        loss_outer, var_list=model_params_outer)
    train_op_outer_end = tf.train.AdamOptimizer(learning_rate_ph, beta1_ph).minimize(
        loss, var_list=model_params_outer)

    train_op_m =  tf.train.AdamOptimizer(0.0002, 0.9).minimize(
        m_loss, var_list=tf.trainable_variables("m"))

    start_train_ops = [train_op_inner_start, train_op_outer_start, train_op_trainer, train_op_m]
    end_train_ops = [train_op_inner_end, train_op_outer_end, train_op_trainer, train_op_m]
...
def gen_lq(img0, size=None):
    ...
    detection = mask * img0 #( mask*img0 + 2*(1-mask)*np.random.rand(*img0.shape)*img )
    lq = -np.ones(img.shape)
    scaled_detection = scale0to1(detection)
    lq[select] = scaled_detection[select]

    

    lq = fill(lq, invalid=np.logical_not(mask.astype(np.bool)))

    loss_scale = poly_fn(np.asarray([use_frac], dtype=np.float32))

    #Changed img to img0 halfway through training
    return img0.astype(np.float32), lq.astype(np.float32), mask.astype(np.float32), loss_scale

def record_parser(record):
    """Parse files and generate lower quality images from them."""

    img = flip_rotate(preprocess(load_image(record)))
    img, lq, mask, loss_scale = gen_lq(img)
    if np.sum(np.isfinite(img)) != cropsize**2 or np.sum(np.isfinite(lq)) != cropsize**2:
        img = np.zeros((cropsize,cropsize))
        lq = mask*img

    return lq, img, mask, loss_scale

def reshaper(img1, img2, img3, loss_scale):
    ...
    loss_scale = tf.reshape(loss_scale, [1])
    return img1, img2, img3, loss_scale
...
def input_fn(dir, subset, batch_size, num_shards):
    ...
    with tf.device('/cpu:0'):
        ...
        dataset = dataset.map(
            lambda file: tf.py_func(record_parser, [file], [tf.float32, tf.float32, tf.float32, tf.float32]),
        ...
        if num_shards <= 1:
            ...
            return [img_batch[0]], [img_batch[1]], [img_batch[2]], [img_batch[3]]
        else:
...
def inference_parser(record):

    lq = load_image(record)
    lq = lq[:cropsize, :cropsize]
    lq = norm_img(lq)
...
def main(job_dir, data_dir, variable_strategy, num_gpus, log_device_placement, 
    ...
    with open(log_file, 'a') as log:
        ...
        with tf.control_dependencies(update_ops):
            ...
            img, img_truth, img_mask, loss_scale = input_fn(data_dir, 'train', batch_size, num_gpus)
            img_val, img_truth_val, img_mask_val, loss_scale_val = input_fn(data_dir, 'val', batch_size, num_gpus)
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                ____img, ____img_truth, ____img_mask, ____loss_scale = sess.run([img, img_truth, img_mask, loss_scale])
                ...
                img_mask_ph = [tf.placeholder(tf.float32, shape=i.shape, name='img_mask')
                                for i in ____img_truth]
                loss_scale_ph = [tf.placeholder(tf.float32, shape=(), name='loss_scale')
                                for i in ____loss_scale]
                ...
                exp_dict = experiment(img_ph[0], img_truth_ph[0], img_mask_ph[0], 
                                      ...
                                      train_outer_ph, ramp_ph, loss_scale_ph[0],
                                      initialize=True)
                ...
                #ckpt_vars = [v for v in tf.global_variables() if not "mse_predicter" in v.name]
                #saver0 = tf.train.Saver(var_list=ckpt_vars)
                ...
                bad_buffer = []
                bad_buffer_mask = []
                bad_buffer_loss_scale = []
                for _ in range(bad_buffer_size):
                    lq, buffer_img, mask, scale = sess.run([img, img_truth, img_mask, loss_scale])
                    bad_buffer_truth.append(buffer_img)
                    bad_buffer.append(lq)
                    bad_buffer_mask.append(mask)
                    bad_buffer_loss_scale.append(scale)
                bad_buffer_prob = 0.2
                ...
                while True:
                    ...
                    while time.time()-time0 < modelSavePeriod:
                        ...
                        if use_buffer:
                            ...
                            _loss_scale = bad_buffer_loss_scale[idx]

                            print("From buffer")
                        else:
                            _img, _img_truth, _img_mask, _loss_scale = sess.run([img, img_truth, img_mask, loss_scale])

                        dict = base_dict.copy()
                        dict.update( { img_ph[0]: _img[0], 
                                       img_truth_ph[0]: _img_truth[0], 
                                       img_mask_ph[0]: _img_mask[0], 
                                       loss_scale_ph[0]: np.float32(float(_loss_scale[0])) } )
                        ...
                        if 0 <= counter <= 1 or not counter % save_result_every_n_batches or (0 <= counter < 10000 and not counter % 1000) or counter == counter_init:
                            ...
                            try:
                                save_input_loc = model_dir+"no_blur_input-"+str(counter)+".tif"
                                save_truth_loc = model_dir+"no_blur_truth-"+str(counter)+".tif"
                                save_output_loc = model_dir+"no_blur_output-"+str(counter)+".tif"
                                save_mask_loc = model_dir+"no_blur_mask-"+str(counter)+".tif"
                        ...
                        message = "NiN-69, Iter: {}, MSE_in: {}, MSE: {}, Wass G: {}, Wass D: {}, Val: {}".format(
