...
modelSavePeriod = 10**10 #Train timestep in hours
modelSavePeriod *= 3600 #Convert to s
model_dir = f"//flexo.ads.warwick.ac.uk/Shared38/ARM200/Users/Jeffrey-Ede/dlss-stem/{EXPER_NUM}/"
...
def record_parser(record, inference=False):
...
def main(job_dir, data_dir, variable_strategy, num_gpus, log_device_placement, 
    ...
    with open(log_file, 'a') as log:
        ...
        img_ph = [tf.placeholder(tf.float32, shape=(1,cropsize,cropsize,1), name='img')]
        img_truth_ph = [tf.placeholder(tf.float32, shape=(1,cropsize,cropsize,1), name='img_truth')]
        img_mask_ph = [tf.placeholder(tf.float32, shape=(1,cropsize,cropsize,1), name='img_mask')]
        ...
        input_ph = img_ph[0]
        output_op = exp_dict["output_outer"]
 
    return sess, input_ph, output_op, base_dict
...
sess, input_ph, output_op, base_dict = main(**vars(args))


def magnifier_net(inputs):

    inputs = scale0to1(inputs)#norm_img(inputs)
    inputs = np.expand_dims(np.expand_dims(inputs, axis=0), axis=-1)

    #############################

    dict = base_dict.copy()
    dict.update( { input_ph: inputs } )

    output = sess.run( output_op, feed_dict=dict )
    
    return output[0,:,:,0]


def gen_lq0(img0, size=None):
...
if __name__ == '__main__':
    ...
    #    if mod > last_mod:
    #        try:
    #            img, _ = read_dm3(filepath)
    #            img = magnifier_net(img)

    #            Image.fromarray(img.astype(np.float32)).save( filepath )
    ...
    addr = "//flexo.ads.warwick.ac.uk/Shared38/ARM200/Users/Jeffrey-Ede/dlss-stem/img.dm3"
    save_file =  "//flexo.ads.warwick.ac.uk/Shared38/ARM200/Users/Jeffrey-Ede/dlss-stem/save.txt"
    save_loc = "//flexo.ads.warwick.ac.uk/Shared38/ARM200/Users/Jeffrey-Ede/dlss-stem/save_data/"
    d = 7

    idx0 = (cropsize%d)//2
    len0 = len([i for i in range((cropsize%d)//2, cropsize, d)])

    target_size = d*len0

    _, lq0, _ = gen_lq0(np.random.rand(cropsize, cropsize), size=d)
    first_size = None
    for i in range(1, d+1):
        if lq0[0,i] != lq0[0,0]:
            first_size = i
            break
    crop_idx0 = d - first_size


    while True:
        #inputs, _ = read_dm4(addr)# = #imread(addr, mode='F')

        try:
            inputs, _ = read_dm4(addr)
        

            x0 = inputs.shape[0]//2 - len0//2
            y0 = inputs.shape[1]//2 - len0//2

            inputs = inputs[x0:x0+len0,y0:y0+len0]

            inputs = cv2.resize(inputs, (target_size, target_size), interpolation=cv2.INTER_NEAREST)[crop_idx0:crop_idx0+cropsize, crop_idx0:crop_idx0+cropsize]

            #inputs = norm_img(inputs)
            #inputs = np.expand_dims(np.expand_dims(inputs, axis=0), axis=-1)

            output = magnifier_net(inputs)

            disp(output, time=10)

            if os.path.isfile(save_file):

                save_filename = save_loc + str(len(os.listdir(save_loc))) + "output.tif"
                Image.fromarray(output.reshape(cropsize, cropsize).astype(np.float32)).save( save_filename )

                save_filename = save_loc + str(len(os.listdir(save_loc))) + "input.tif"
                Image.fromarray(inputs.reshape(cropsize, cropsize).astype(np.float32)).save( save_filename )
    ...
    #        x0 = inputs.shape[0]//2 - 52
    #        y0 = inputs.shape[1]//2 - 52

    #        inputs = inputs[x0:x0+103,y0:y0+103]

    #        inputs = cv2.resize(inputs, (515, 515), interpolation=cv2.INTER_NEAREST)[1:-2, 1:-2]
    ...
    #        if os.path.isfile(save_file):
    #            save_filename = save_loc + str(len(os.listdir(save_loc))) + ".tif"
    #            Image.fromarray(output.reshape(cropsize, cropsize).astype(np.float32)).save( save_filename )
    #            os.remove(save_file)
