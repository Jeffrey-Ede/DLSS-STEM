...
def record_parser(record, inference=False):
    ...
    #addr = r"\\flexo.ads.warwick.ac.uk\Shared38\ARM200\Users\Richard\2017\2017-08-25_TECS-1\TECS-1A (1)\JEOL BF__200kV_60kX_2305.tif"
    #addr = r"Z:\Jeffrey-Ede\models\stem-random-walk-nin-20-54\truth-1200000.tif"
    #addr = r"Z:\Jeffrey-Ede\models\stem-random-walk-nin-20-54\truth-1125000.tif"
    #addr = r"Z:\Jeffrey-Ede\models\stem-random-walk-nin-20-54\truth-500000.tif"
    addr = r"Z:\Jeffrey-Ede\models\stem-random-walk-nin-20-54\test_output-18.tif"

    img = imread(addr, mode="F")
    img = preprocess(img)
    small_size = 103
    img = img[(img.shape[0]-small_size)//2:(img.shape[0]-small_size)//2+small_size,
              (img.shape[1]-small_size)//2:(img.shape[1]-small_size)//2+small_size]

    upsampled = np.zeros((cropsize, cropsize))
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                while True:
                    ...
                    while time.time()-time0 < modelSavePeriod:
                        ...
                        #_img_ph[0], _ = record_parser()

                        if counter < max_count:
                            print("Iter: ", counter)
                            output = sess.run( exp_dict["output_outer"], feed_dict=dict )
                            #mses.append(mse)
                            ...
                            counter = 5
                            save_input_loc = model_dir+"richard_input-"+str(counter)+".tif"
                            save_truth_loc = model_dir+"richard_truth-"+str(counter)+".tif"
                            save_output_loc = model_dir+"richard_output-"+str(counter)+".tif"
                            save_mask_loc = model_dir+"richard_mask-"+str(counter)+".tif"
                            ...
                            Image.fromarray((0.5*output+0.5).reshape(cropsize, cropsize).astype(np.float32)).save( save_output_loc )
                        ...
                        message = "NiN-54, Iter: {}, MSE_in: {}, MSE: {}, Wass G: {}, Wass D: {}, Val: {}".format(
