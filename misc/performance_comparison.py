...
def main(job_dir, data_dir, variable_strategy, num_gpus, log_device_placement, 
    ...
    with open(log_file, 'a') as log:
        ...
        img, img_truth, img_mask = input_fn(data_dir, 'train', batch_size, num_gpus)
        img_val, img_truth_val, img_mask_val = input_fn(data_dir, 'val', batch_size, num_gpus)
...
#    addr = model_dir + "input-975000.tif"
#    inputs = imread(addr, mode='F')

#    inputs = norm_img(inputs)
#    inputs = np.expand_dims(np.expand_dims(inputs, axis=0), axis=-1)

#    output = magnifier(inputs)
...
MAX_FILES = 20_000
PARENT_DIR = "//Desktop-sa1evjv/f/ARM_scans-crops/test/"
SAVE_DIR = "//flexo.ads.warwick.ac.uk/Shared41/Microscopy/Jeffrey-Ede/models/"
...
for i, file in enumerate(files):
    ...
    for m in [4, 5, 6, 8, 10]:
        ...
        c = 512-512%m
        sparse = cv2.resize(img[:c,:c], (s,s), interpolation=cv2.INTER_NEAREST)
    
        r0 = cv2.resize(sparse, (c,c), interpolation=cv2.INTER_NEAREST)
        r1 = cv2.resize(sparse, (c,c), interpolation=cv2.INTER_AREA)
        r2 = cv2.resize(sparse, (c,c), interpolation=cv2.INTER_LINEAR)
        r3 = cv2.resize(sparse, (c,c), interpolation=cv2.INTER_CUBIC)
        r4 = cv2.resize(sparse, (c,c), interpolation=cv2.INTER_LANCZOS4)

        #_, lq, _ = gen_lq(img)
        ##lq = cv2.resize(sparse, (515, 515), interpolation=cv2.INTER_NEAREST)[1:-2, 1:-2]
        #r5 = 0.5*magnifier_net(lq) + 0.5

        img0 = cv2.GaussianBlur(img[:c,:c], (5, 5), sigmaX=2.5, sigmaY=2.5)

        #print("shapes", r0.shape, img0.shape, c)
        #disp(r3)
        #disp(r5)
        #disp(img)

        e0 = np.mean( (r0 - img0)**2 )
        e1 = np.mean( (r1 - img0)**2 )
        e2 = np.mean( (r2 - img0)**2 )
        e3 = np.mean( (r3 - img0)**2 )
        e4 = np.mean( (r4 - img0)**2 )
        #e5 = np.mean( (r5 - img)**2 )

        me = np.asarray([e0, e1, e2, e3, e4])
        mes.append(me)
    ...
    if not i % 100:
        errors_stack = np.stack(errors)
        np.save(SAVE_DIR+f"dlss-stem_perf_comp.npy", errors_stack)