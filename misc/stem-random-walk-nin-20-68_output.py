...
model_dir = "//flexo.ads.warwick.ac.uk/Shared41/Microscopy/Jeffrey-Ede/models/stem-random-walk-nin-20-68/"
...
poly = np.poly1d(np.load(model_dir+"poly.npy"))
...
def experiment(feature, ground_truth, mask, learning_rate_ph, discr_lr_ph, beta1_ph, 
    ...
    mse_inner = capper_fn(mse_inner/loss_scale_ph)
    ...
    mse0 = mse_outer
    mse_outer = capper_fn(mse_outer/loss_scale_ph)
    ...
    mse_outer_together = capper_fn(mse_outer_together/loss_scale_ph)
...
def gen_lq(img0, size=None):
    ...
    if size == None:
        size = 10#np.random.randint(4, 11)
    use_frac = 1/size**2
...
def main(job_dir, data_dir, variable_strategy, num_gpus, log_device_placement, 
    ...
    with open(log_file, 'a') as log:
        ...
        with tf.control_dependencies(update_ops):
            ...
            img, img_truth, img_mask, loss_scale = input_fn(data_dir, 'test', batch_size, num_gpus)
            img_val, img_truth_val, img_mask_val, loss_scale_val = input_fn(data_dir, 'test', batch_size, num_gpus)
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                counter = 0#1_555_162#1_429_051#1_363_555#1_298_542#1_272_142#1_214_115#1_174_742#1_117_006#1_000_000#0
                val_counter = 0
                ...
                while True:
                    ...
                    while time.time()-time0 < modelSavePeriod:
                        ...
                        filepath = model_dir + f"final_25_truth-{counter}.tif"
                        _, lq, _, _ = gen_lq(preprocess(load_image(filepath)))
                        _img[0] = np.expand_dims(np.expand_dims(lq, 0), -1)
                        ...
                        if True and counter < max_count:#if 0 <= counter <= 1 or not counter % save_result_every_n_batches or (0 <= counter < 10000 and not counter % 1000) or counter == counter_init:
                            ...
                            try:
                                #save_input_loc = model_dir+"final_100_input-"+str(counter)+".tif"
                                #save_truth_loc = model_dir+"final_49_truth-"+str(counter)+".tif"
                                save_output_loc = model_dir+"final_100_adversarial-"+str(counter)+".tif"
                                #save_mask_loc = model_dir+"final_49_mask-"+str(counter)+".tif"
                        ...
                        message = "NiN-68, Iter: {}, MSE_in: {}, MSE: {}, Wass G: {}, Wass D: {}, Val: {}".format(
