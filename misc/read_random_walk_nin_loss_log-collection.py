...
sets = [[1, 2, 3], [5, 4, 23], [1, 4, 6, 7], 
        ...
        [33, 33], [38, 39, 36, 43, 44, 46], [39],
        [38, 52, 36, 43], [66, 54, 65], [2, 54]]
...
for i, (data_nums, labels) in enumerate(zip(sets, labels_sets)):
    ...
    for j, dataset_num in enumerate(data_nums):
        if i == 17 and dataset_num == 2:
            log_loc = "Z:/Jeffrey-Ede/models/magnifier/"
            mse_indicator = "Loss:"
            log_file = log_loc+f"log-{dataset_num}.txt"
        ...
        with open(log_file, "r") as f:
            ...
            if dataset_num < 14 and not i == 17:
                for line in f:
        ...
        if i in [14, 15]:
            losses_iters = losses_iters[:487_500]
            losses = losses[:len(losses_iters)]
        if i in [16] and dataset_num == 54:
            losses_iters = losses_iters[:990_000]
            losses = losses[:len(losses_iters)]
        if i == 17:
            if dataset_num == 54:
                losses_iters = losses_iters[:997_000]
                losses = losses[:len(losses_iters)]
                losses = [x / (3.5/2) for x in losses]
            l = len(losses_iters)/1000
            losses_iters = [100*x/l for x in losses_iters if x/l > 0.01]
            losses = losses[-len(losses_iters):]
    ...
    if i in [17]:
        plt.axvline(x=0.01, color='black', linestyle='--', linewidth=0.8)
    ...
    if i in [15]:
        ...
        name = str(i+1)
    save_loc =  "Z:/Jeffrey-Ede/models/stem-random-walk-nin-figures/" + name + ".png"
